...
import { ArrowLeft, Save, Sparkles, CheckCircle2, Loader2, Key } from "lucide-react";
...
export default function AdminNewSeoPage() {
  ...
  const [formData, setFormData] = useState({
    pagePath: "/services/custom-seo",
    seoTitle: "Custom Service Title | Rizwan Saeed",
    metaDescription:
      "Comprehensive digital marketing service meta description optimized for Google search rankings and social share CTR.",
    focusKeyword: "Digital Marketing Dubai",
    canonicalUrl: "https://rizwansaeed.com/services/custom-seo",
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("custom_seo_settings_data", JSON.stringify([newItem, ...existing]));
    ...
    <div className="space-y-8 max-w-4xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-bold text-slate-900 font-serif">
            Create Custom Page SEO (Rank Math Pro)
          </h1>
          <p className="text-xs text-slate-500">
            Configure focus keywords, SERP meta tags, OpenGraph cards, and JSON-LD schemas.
          </p>
        </div>
      ...
      {success && (
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-900 flex items-center gap-2 text-sm font-bold animate-fade-in">
          ...
          <span>SEO settings saved successfully! Redirecting...</span>
        </div>
      )}
